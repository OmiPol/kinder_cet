...
class Puzzle_move(Node):
    def __init__(self):
        ...
        self.namespaceparam = self.declare_parameter('namespace',"/sim")
        ...
        self.pos = self.create_publisher(Pose,self.namespace+"/position",10)
        self.pub = self.create_publisher(JointState,'/sim/joint_states',10)
        self.wheels = self.create_publisher(WheelV,"/wheel_velocities",10)
    ...
    def math_callback(self):
        ...
        self.tfin = time.time()
        rwv = WheelV() #randomized wheel velocity
        msg = Pose()
        if self.vels is not None:
            ...
            if(wL != 0 and wR != 0): #si las llantas se están moviendo se introduce el error
                rwv.left_vel = wL #+  (random.random()*0.1) +0.05
                rwv.right_vel = wR #+ (random.random() * 0.1) +0.05
            else:
                rwv.left_vel = 0.0
                rwv.right_vel = 0.0
                
            self.wheels.publish(rwv) #pública velocidad de las llantas
